...
function activate(context) {
	let disposable = vscode.commands.registerCommand('JtoD.JtoD', async function () {
		...
		try {
			...
			const className = await vscode.window.showInputBox({
				...
				value: "AutoGeneratedClassName"
			})
			const dartData = new JsonToDartClassInfo(json, className).result
			const dart = generateClass(dartData)
			editor.edit(builder => {
				builder.replace(new vscode.Range(editor.document.lineAt(0).range.start, editor.document.lineAt(editor.document.lineCount - 1).range.end), dart);
			});
		} catch (error) {
			console.log(error);
